...
for i=1: size(final_each_recommandation_collection_question,1)
    ...
    if size(learner_assignment,1)==0
        ...
        final_each_recommandation_collection_question_Euclid{i,1}=[];
        %按照曼哈顿距离推荐作业
        final_each_recommandation_collection_question_Manhattan{i,1}=[];
        %按照cos距离推荐作业
        b=sortrows(distance_similarity,1);
        final_each_recommandation_collection_question_cos{i,1}=[];
    else
...
%     c_max_l=(c_max_h+c_min_l)/2+(c_max_h-c_min_l)/4;
    ...
    %推荐的知识点
     if size(each_student_knowledege_matrix{i,2},1)==0
        ...
        %计算推荐的知识点和别人作业中包含知识点的相似性, 难度之间的差值
        similarity_matrix=[];
        distance_similarity=[];
        for j=1:size(final_each_recommandation_collection_question,1)
            if j==i
                similarity_matrix(j,1)=0;
                similarity_matrix(j,2)=0;
                similarity_matrix(j,3)=1;
                distance_similarity(j,1)=+Inf;
                distance_similarity(j,2)=+Inf;
                distance_similarity(j,3)=+Inf;
            else
                if size(final_each_recommandation_collection_question{j,1},1)~=0
                    middle=final_each_recommandation_collection_question{j,1};
                    mean_difficulty=mean(cell2mat(middle(:,6)));
                    middle=middle(:,2:4);
                    middle_knowledge_item=string(zeros(10,3));
                    for k=1:size(middle_knowledge_item,1)
                        for t=1:size(middle_knowledge_item,2)
                            middle_knowledge_item(k,t)=cell2mat(middle(k,t));
                        end
                    end
                    ...
                    count=0;
                    for number_1=1:size(middle_knowledge_item,1)
                        for number_2=1:size(recommended_knowledge_item,1)
                            if strcmp (middle_knowledge_item(number_1,1),recommended_knowledge_item(number_2,1))==1
                                count=count+1;
                                break
                            end
                        end
                    end
                    %计算知识点，难度和别人作业的相似性，第一列欧几里得，第二列曼哈顿，第三列cos
                    similarity_matrix(j,1)=count/size(recommended_knowledge_item,1);
                    similarity_matrix(j,2)=mean_difficulty;
                    similarity_matrix(j,3)=recommended_difficulty;
                    %欧几里得距离
                    distance=(1-similarity_matrix(j,1))^2+(similarity_matrix(j,2)-similarity_matrix(j,3))^2;
                    distance=sqrt(distance);
                    distance_similarity(j,1)=distance;
                    %曼哈顿距离
                    distance=abs((1-similarity_matrix(j,1)))+abs(similarity_matrix(j,2)-similarity_matrix(j,3));
                    distance_similarity(j,2)=distance;
                    %Cosine Similarity
                    X=[1,similarity_matrix(j,3)];
                    Y=[similarity_matrix(j,1),similarity_matrix(j,2)];
                    distance=dot(X,Y)/(norm(X)*norm(Y));
                    distance_similarity(j,3)=1-distance;
                    
                    distance_similarity(j,4)=j;
                else
                    distance_similarity(j,1)=+Inf;
                    distance_similarity(j,2)=+Inf;
                    distance_similarity(j,3)=+Inf;
        ...
        %按照欧几里得距离推荐作业
        b=sortrows(distance_similarity,1);
        final_each_recommandation_collection_question_Euclid{i,1}=final_each_recommandation_collection_question{b(1,4),1};
        %按照曼哈顿距离推荐作业
        b=sortrows(distance_similarity,2);
        final_each_recommandation_collection_question_Manhattan{i,1}=final_each_recommandation_collection_question{b(1,4),1};
        %按照cos距离推荐作业
        b=sortrows(distance_similarity,3);
        final_each_recommandation_collection_question_cos{i,1}=final_each_recommandation_collection_question{b(1,4),1};
        %计算推荐难度和知识点难度之间的差距，1列欧几里得距离，2列曼哈顿距离，3cos距离
        middle=final_each_recommandation_collection_question_Euclid{i,1};
        mean_difficulty=mean(cell2mat(middle(:,6)))/max_difficulty;
